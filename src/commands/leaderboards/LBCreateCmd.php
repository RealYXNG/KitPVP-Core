...
class LBCreateCmd extends Command{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		if($sender instanceof Player) {
			if(!isset($args[0])) {
				$sender->sendMessage("§cUSAGE: /createleaderboard §7<Leaderboard Name> <Leaderboard Type> §c{0 - Kills, 1 - Deaths, 2 - KDR, 3 - XP, 4 - Levels, 5 - Coins}");
				return;
			}
			...
			if(!is_numeric($args[1])) {
				$sender->sendMessage("§cArgument <Leaderboard Type> must be of {0 - Kills, 1 - Deaths, 2 - KDR, 3 - XP, 4 - Levels, 5 - Coins}");
				return;
			}
